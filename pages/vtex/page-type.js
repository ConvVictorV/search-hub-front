...
import { Container, FlexboxGrid, Panel, Row, Form, Input, Button, useToaster, Message  } from "rsuite";
        ...
        <FlexboxGrid justify="center" align="middle" style={{ height: "100%" }}>
          <FlexboxGrid.Item colspan={24}>
            ...
            >
              <Row>
                <Form>
                  <Input as="textarea" rows={3} placeholder={`Insira as urls separadas por enter\nhttps://exemplo/infantil/jordan\nhttps://exemplo/tenis/infantil`} onChange={value=>{
                    setUrls(value.split("\n"))
                  }}></Input>
                  <Input as="textarea" style={{marginTop:"20px"}} readOnly rows={3} placeholder={`Resultado`} value={result}></Input>
                  <Button appearance="primary" type="submit" 
                  onClick={()=>{
                    setResult("")
                    setLoading(true)
                    axios.post("/api/post/vtexType",{urls})
                    .then((result)=>{
