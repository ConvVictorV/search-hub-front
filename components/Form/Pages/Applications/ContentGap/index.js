...
function FormComponent({ footer, sendText, ...rest }) {
  ...
  const sendData = async () => {
    ...
    axios
      .get(
        "https://southamerica-east1-iconic-rampart-279113.cloudfunctions.net/post_content_gap",
        {
          headers: {
  ...
  return (
    <Form {...rest} fluid>
      ...
      >
        ...
        >
          Clique ou arraste os arquivos para fazer upload
        </div>
      </Uploader>

      <Select
        placeholder="Conta Search Console"
        fetch="/api/get/select/customersName"
        onSelect={setAccount}
      />
      <TagInput
        trigger={["Enter"]}
