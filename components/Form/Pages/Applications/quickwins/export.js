...
function FormComponent({ data, closeModal, footer, sendText, ...rest }) {
        ...
        clearToast().then(() => {
            ...
            toast.push(
                <Notification type="info" header="Download pronto" closable duration={0} style={{ width: 320 }}>
                    <hr />
                    <a href={url} download={`${projectCustomer || 0}_export_quickwins_${new Date().getTime()}.csv`}><Button>Download</Button></a>
                </Notification>
                , { placement: "topCenter" }
            )
        })
    ...
    const sendData = async () => {
        ...
        if (exportData.length == 0) {
            await axios.get(projectCustomer ? `/api/get/exportQuickWins?idcustomer=${projectCustomer}` : `/api/get/exportQuickWins`)
            ...
            await axios.post(`/api/get/exportQuickWins`, {
                content: exportData
            })
                .then(({ data }) => downloadData(data))
                .catch((e) => {
